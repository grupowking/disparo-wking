// ====================================================================
//  Via Búzios – Disparo “Final com Retentativa Paciente e Prioritária”
// ====================================================================
...
const MAX_RETRIES_MASSA = 3; // Número máximo de tentativas para disparos em massa
const MAX_RETRIES_RESPOSTA = 2; // Número máximo de tentativas para respostas interativas
const RETRY_DELAY_RESPOSTA = 25_000; // Pausa de 25s antes de tentar reenviar uma resposta
...
fs.createReadStream('contatos.csv')
  ...
  .on('data', (row) => {
    // SUA MELHORIA: Adicionado `row['\uFEFFnome']` para lidar com BOM do Excel.
...
// ======================= SISTEMA DE FILA COM RETENTATIVA PACIENTE E PRIORITÁRIA =======================
const messageQueue = [];
    ...
    while (messageQueue.length > 0) {
        ...
        try {
            if (!dentroDoHorario() && job.isMassMessage) {
                ...
                await delay(600_000); // Pausa longa de 10 min
                continue;
            }
            ...
            // --- LÓGICA DE RETENTATIVA DIFERENCIADA ---
            job.retryCount = (job.retryCount || 0) + 1;
            const isChatNotFound = e.message && e.message.includes('Chat not found');

            if (isChatNotFound) {
                if (job.isMassMessage) {
                    // Lógica para DISPARO EM MASSA: Paciência.
                    if (job.retryCount < MAX_RETRIES_MASSA) {
                        console.log(`[FILA] 🟡 Falha "Chat not found" (Tentativa ${job.retryCount}/${MAX_RETRIES_MASSA}). Devolvendo para o FIM da fila para uma longa pausa.`);
                        messageQueue.push(job); // Adiciona de volta no FIM da fila.
                    } else {
                        console.log(`[FILA] ❌ Falha definitiva para ${job.logInfo} após ${MAX_RETRIES_MASSA} tentativas. Job descartado.`);
                    }
                } else {
                    // Lógica para RESPOSTA INTERATIVA: Agilidade.
                    if (job.retryCount < MAX_RETRIES_RESPOSTA) {
                        ...
                        messageQueue.unshift(job); // Devolve para o INÍCIO da fila.
                        await delay(RETRY_DELAY_RESPOSTA); // Faz uma pausa antes da próxima tentativa.
                    } else {
                         console.log(`[FILA] ❌ Falha definitiva para ${job.logInfo} após ${MAX_RETRIES_RESPOSTA} tentativas. Job descartado.`);
                    }
                }
            } else {
                // Para qualquer outro erro, descarta o job para não entrar em loop.
                console.log(`[FILA] ❌ Falha definitiva para ${job.logInfo} (Erro não recuperável: ${e.message}). Job descartado.`);
            }

        } finally {
            try { await client.stopTyping(job.to); } catch (e) {}
            // Pausa mínima entre cada job para garantir estabilidade
            if (job.isMassMessage) {
                await delay(1000); 
            }
        }
    ...
    try {
        ...
        client.onMessage(async (msg) => {
            ...
            if (RESPOSTAS_OK.some(pat => txt.includes(pat))) {
                ...
                // Adiciona na ordem inversa (primeiro o vídeo, depois o texto) para que o texto seja processado antes
                messageQueue.unshift({ type: 'file', to: msg.from, path: VIDEO_PATH, filename: 'cupom.mp4', caption: '🎁 Aproveite essa surpresa da Via Búzios com carinho!', humanDelay: rnd(VIDEO_DELAY_MIN, VIDEO_DELAY_MAX), logInfo: `resposta de vídeo para ${primeiroNome}`, isMassMessage: false, retryCount: 0 });
        ...
        for (const c of contatos) {
            ...
            messageQueue.push({ type: 'text', to: c.telefone, content: txt, humanDelay: rnd(INTERVALO_MIN, INTERVALO_MAX), logInfo: `disparo para ${c.nomeCompleto}`, isMassMessage: true, retryCount: 0 });
        }
